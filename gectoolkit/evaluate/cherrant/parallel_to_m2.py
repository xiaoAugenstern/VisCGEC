...
def annotate(line):
    ...
    for idx, target in enumerate(sent_list[1:]):
        ...
        # Print source and target to debug 24/10/02
        # print(f"Processing idx: {idx}, source: '{source}', target: '{target}'")
        if source not in sentence_to_tokenized:
            print(f"KeyError: Source '{source}' not found in sentence_to_tokenized")
        if target not in sentence_to_tokenized:
            print(f"KeyError: Target '{target}' not found in sentence_to_tokenized")
