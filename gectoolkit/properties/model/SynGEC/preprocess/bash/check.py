...
def check_two_space(file_path):
    with open(file_path, 'r') as file:
        ...
        for line_number, line in enumerate(file, start=1):
            ...
            if '  ' in line:
                pos = True
                # print(f"第 {line_number} 行存在连续的两个空格。 {line}")
        if pos==False:
...
def clean_and_replace(file_path, out_path):
    ...
    with open(out_path, 'w', encoding='utf-8') as output_file:
        any_changes = False  # 用于记录是否有任何更改发生
    ...
    if any_changes:
        ...
        print(f'文件 {file_path} 中未发现需要清除的特殊字符或多余的点号，文件内容保持不变，并已保存到 {out_path}。')
...
def find_blank_lines(file_path):
    pos = True
    with open(file_path, 'r', encoding='utf-8') as file:
        for line_number, line in enumerate(file, start=1):
            # strip() 方法用来移除字符串头尾指定的字符（默认为空格或换行符）
            if not line.strip():
                print(f"Blank line found at line {line_number}")
                pos = False
    return pos

def all_check(file_path,out_path):
    print(f'\n================ 处理{file_path} ==============')
    failed_steps = []
    pattern = ' '
    # （1）去除脏数据：
    ...
    # (12) 检查是否存在空行
    find_blank_lines(out_path)
    # （13）最后检查
    if not check_lines_starting_with(pattern, out_path):
        print(f'\n文件 {file_path} 在步骤 13.1 检查开头是否有空格失败')
        return False

    if not check_fullwidth_space(out_path):
        print(f'\n文件 {file_path} 在步骤 13.2 检查是否有全角空格失败')
        return False

    if not check_two_space(out_path):
        print(f'\n文件 {file_path} 在步骤 13.3 检查是否两个连续空格失败')
        return False

    if not find_blank_lines(out_path):
        print(f'\n文件 {file_path} 在步骤 13.4 检查是否存在空行失败')
        return False

    print(f'\n文件 {file_path} 所有步骤成功通过')
    return True
